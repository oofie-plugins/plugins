...
@Slf4j
@Singleton
class OofieKhazardMinerOverlay extends OverlayPanel
{
    private final Client client;
    private final OofieKhazardMinerPlugin plugin;
    private final OofieKhazardMinerConfig config;

    String timeFormat;
    private String infoStatus = "Starting...";

    @Inject
    private OofieKhazardMinerOverlay(final Client client, final OofieKhazardMinerPlugin plugin, final OofieKhazardMinerConfig config)
    {
        super(plugin);
        ...
        this.plugin = plugin;
        this.config = config;
        getMenuEntries().add(new OverlayMenuEntry(RUNELITE_OVERLAY_CONFIG, OPTION_CONFIGURE, "OofieKhazardMiner overlay"));
    }
    ...
    public Dimension render(Graphics2D graphics)
    {
        ...
        tableDelayComponent.addRow("Sleep delay:", OofieKhazardMinerPlugin.sleepLength + "ms");
        ...
        if (!tableComponent.isEmpty())
        {
            ...
            panelComponent.getChildren().add(TitleComponent.builder()
                .text("Oofie's Khazard Miner 2.0.0")
