...
@Slf4j
@Singleton
class superheatOverlay extends OverlayPanel
{
	private final Client client;
	private final superheatPlugin plugin;
	private final superheatConfig config;

	String timeFormat;
	private String infoStatus = "Starting...";

	@Inject
	private superheatOverlay(final Client client, final superheatPlugin plugin, final superheatConfig config)
	{
		super(plugin);
		...
		this.plugin = plugin;
		this.config = config;
		getMenuEntries().add(new OverlayMenuEntry(RUNELITE_OVERLAY_CONFIG, OPTION_CONFIGURE, "Oofie's Superheat Overlay"));
	}
	...
	public Dimension render(Graphics2D graphics)
	{
		...
		tableDelayComponent.addRow("Sleep delay:", superheatPlugin.sleepLength + "ms");
